...
def all_evaluate(logits,target):
    #print(target,output)
    lengh=len(logits)
    scores = torch.sigmoid(logits)
    # auroc = roc_auc_score(target, scores)
    auroc = roc_auc_score(target, scores, multi_class='ovr')
    ...
    for i in range(lengh):
        if (scores[i].all() < ave_scores.all()):
            scores[i] = 0
        else:
            scores[i] = 1
...
def feed_train_net(net,dataloader,predictor,bcewithlogitsloss, criterion):
...
def feed_val_net(net,dataloader,predictor, bcewithlogitsloss,criterion):
...
def feed_test_net(net,dataloader,predictor,bcewithlogitsloss, criterion):
...
def evaluate_net(net,train_dataloader, validation_dataloader, test_dataloader,predictor,bcewithlogitsloss, criterion, args):
    global g
    ...
    train_output, train_loss, train_target = feed_train_net(net,g.train_subset_loader,predictor,bcewithlogitsloss, criterion)
    validation_output, validation_loss, validation_target = feed_val_net(net,validation_dataloader, predictor,bcewithlogitsloss, criterion)
    test_output, test_loss, test_target = feed_test_net(net,test_dataloader,predictor,bcewithlogitsloss, criterion)
