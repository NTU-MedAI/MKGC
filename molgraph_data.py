...
class MolGraphDataset(data.Dataset):
    def __init__(self, path, prediction=False):
        ...
        if prediction:
            ...
            self.targets = np.genfromtxt(path, delimiter=',', skip_header=1, usecols=[8],dtype=np.int, comments=None)
    ...
    def __getitem__(self, index):
        ...
        return (adj_1, nd_1, ed_1), targets, d1, mask_1
...
def molgraph_collate_fn(data):
    n_samples = len(data)
    (adj_1, node_fts_1, edge_fts_1),targets_0,d1,mask_1= data[0]
    ...
    for i in range(n_samples):
        (adj_1, node_fts_1, edge_fts_1), target, d1, mask_1 = data[i]
    ...
    return adjacency_tensor_1, node_tensor_1, edge_tensor_1,target_tensor,d1_emb_tensor,mask_1_tensor
